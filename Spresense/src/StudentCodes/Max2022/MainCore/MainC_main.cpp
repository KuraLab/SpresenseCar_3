...
namespace MAX2022
{
    ...
    void MainCore_loop() {
        ...
        int8_t msgid;
        uint32_t msgdata;
        MP.Recv(&msgid, &msgdata, SUBCORE_2_ID);
        switch (msgid)
        {
            case C2_T3_NO_PEAK:{
    ...
    unsigned int FFT_PikaPika_Routine(){
        ...
        for (int i = 0; i < PIKA_SENSOR_COUNT; i++)
        {
            sum_dphi += dphi[i];
        ...
        {
            PikaPika_LED_countdown = DEFAULT_PIKA_BLINK_TIME;
            for (int i = 0; i < PIKA_SENSOR_COUNT; i++)
            {
            if (PikaPika_light_sensor_life[i] > 0)
            {
        ...
        if (FFT_countdown == 0)
        {
            ...
            MP.Send(C2_T0_FFT, (uint32_t)(FFT_MSG_SCALE * phi + FFT_MIDSHIFT), SUBCORE_2_ID);
            MP.Send(C2_T1_DXPHI, (uint32_t)(FFT_MSG_SCALE * dx_phi + FFT_MIDSHIFT), SUBCORE_2_ID);
            MP.Send(C2_T2_DYPHI, (uint32_t)(FFT_MSG_SCALE * dy_phi + FFT_MIDSHIFT), SUBCORE_2_ID);
        }
        //****** サブコアへの位相通知処理ここまで *********
