    ...
    if (addressed == I2CSlave::WriteAddressed) {
        ...
        switch (buffer[0]) {
            case '0':{  // PWM
                ...
                i2c_input->pwm_dir = I2C_Receive_PWM_DIR(buffer);
                break;
            }
            case '1':{  // Target XY
                i2c_input = new I2C_Input();
                i2c_input->input_type = TargetXY;
                i2c_input->targetXY = I2C_Receive_TargetXY(buffer);
                break;
            }
            case '2':{  // Radian
                i2c_input = new I2C_Input();
                i2c_input->input_type = Radian;
                i2c_input->radian = I2C_Receive_Radian(buffer);
                break;
            }
            case '3':{  // Encoder
                i2c_input = new I2C_Input();
                i2c_input->input_type = Encoder;
                i2c_input->encoder = I2C_Receive_Encoder(buffer);
                break;
            }
            default:break;
        }
